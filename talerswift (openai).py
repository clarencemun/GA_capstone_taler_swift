...
def chat_with_model(input_text, language):
    ...
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",  # You can replace it with "gpt-4-1106-preview" if using GPT-4
...
def generate_images_from_story(story_text):
    # Create a timestamped directory for this story's images
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
